...
class MDPAgent(Agent):
    ...
    def getRewards(self, state):
        ...
        self.BlankPositions = list(
            set(self.Rewards.keys())
            - set(self.FoodPositions)
            - set(self.WallPositions)
            - set(self.GhostPositions)
            - set(self.CapsulePositions)
        )
        ...
        self.Rewards.update(
            {capsule: self.CAPSULE_REWARD for capsule in self.CapsulePositions}
        )
        ...
        # Otherwise, we get greedy.
            ...
            if DangerZone:
                self.Rewards.update(
                    {danger: -self.GHOST_NEIGH for danger in DangerZone}
                )
            self.Rewards.update({ghost: -self.GHOST_REWARD for ghost in Ghosts})
    ...
    def getPossibleStates(self, state, NewState, action=None):
        ...
        if NewState in Walls:
            ...
            if (
                min(self.Corners)[0] < NewState[0] < max(self.Corners)[0]
                and min(self.Corners)[1] < NewState[1] < max(self.Corners)[1]
            ):
                Escape = {}
        ...
        if action == Directions.NORTH or action == Directions.SOUTH:
            PossibleStates[action] = (
                PossibleDirections[action]
                if PossibleDirections[action] not in Walls
                else NewState
            )
